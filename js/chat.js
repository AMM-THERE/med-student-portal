  ...
  function render() {
    ...
    view.innerHTML = `
      <div class="max-w-5xl mx-auto space-y-4 pb-12 animate-fade-in">
        ...
        <!-- Header -->
        <div class="bg-slate-800/40 p-5 rounded-2xl border border-slate-700/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
          ...
          <div class="flex items-center gap-3 shrink-0">
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-slate-800 text-slate-300 border border-slate-700">
              <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
              Live discussion
            </span>
            <span class="px-3 py-1 rounded-full text-xs font-semibold bg-brand-900/40 text-brand-300 border border-brand-800/50">${messages.length} messages</span>
          </div>
        </div>

        <!-- Pinned Messages Banner -->
        ${pinnedMessages.length > 0 ? `
          <div class="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-3 px-4 flex items-center justify-between text-xs text-amber-200 gap-3">
        ...
        <div class="bg-slate-900/80 rounded-2xl border border-slate-800 shadow-xl flex flex-col h-[560px]">
            ...
            ` : messages.map(msg => {
              ...
              return `
                <div class="flex gap-3 ${isMe ? 'flex-row-reverse' : 'flex-row'} items-start group">
                  <div class="w-8 h-8 rounded-full ${isMe ? 'bg-brand-600' : 'bg-slate-700'} text-white font-bold flex items-center justify-center text-xs shrink-0 shadow-sm mt-1">
                  ...
                  <div class="max-w-[75%] ${isMe ? 'items-end' : 'items-start'} flex flex-col">
                    ...
                    <div class="relative p-3.5 rounded-2xl text-sm ${isMe ? 'bg-brand-600 text-white rounded-tr-none' : 'bg-slate-800 text-slate-200 rounded-tl-none border border-slate-700/60'} shadow-sm leading-relaxed whitespace-pre-wrap break-words">
                      ...
                      <!-- Replying Context -->
                      ${parentMsg ? `
                        <div class="mb-2 p-2 rounded-lg bg-black/20 border-l-2 border-brand-400 text-xs opacity-85">
          ...
          <!-- Active Reply Bar -->
          <div id="reply-preview-bar" class="${currentReplyTo ? 'flex' : 'hidden'} items-center justify-between px-4 py-2 bg-slate-800/90 border-t border-slate-700 text-xs text-slate-300">
          ...
          <div id="file-preview-bar" class="${attachedFile ? 'flex' : 'hidden'} items-center justify-between px-4 py-2 bg-slate-800/90 border-t border-slate-700 text-xs text-slate-300">
            <div class="flex items-center gap-2 truncate">
              <span>📎 Attached:</span>
          ...
          <!-- Input Footer -->
          <div class="p-3 border-t border-slate-800 bg-slate-900/50 rounded-b-2xl">
            <div class="flex items-center gap-2">
              
              <!-- Attachment Button -->
              <label class="p-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl cursor-pointer transition border border-slate-700" title="Attach Image or File">
                <span class="text-sm">📎</span>
                <input type="file" id="chat-file-input" class="hidden" accept="image/*,.pdf,.doc,.docx,.txt" />
              </label>

              <input type="text" id="chat-input" placeholder="Type a message..." class="flex-1 bg-slate-800/80 border border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-brand-500 transition placeholder:text-slate-500" />
              
              <button id="chat-send-btn" class="px-5 py-2.5 bg-brand-600 hover:bg-brand-500 active:scale-95 text-white font-bold text-sm rounded-xl transition flex items-center gap-1.5 shadow-md shrink-0 cursor-pointer">
                <span>Send</span>
                <span class="text-xs">🚀</span>
              </button>
            </div>
            
            <div class="mt-2.5 flex items-center justify-between px-1 text-xs text-slate-400">
